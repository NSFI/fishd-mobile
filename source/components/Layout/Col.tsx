...
class Col extends React.Component<ColProps, any> {
  ...
  render() {
    const { children, tag, className, style, prefixCls, span, offset, onClick, gutter }: ColProps = this.props;
    ...
    return (
      <CustomTag tag={tag} style={Object.assign({}, style, gutterStyle)} className={wrapCls} onClick={onClick}>
        {children}
      </CustomTag>
    );
  }
